...
int SnpeDetector::executeSnpeObjectDetector(const uint8_t* rawData, int w, int h, std::vector<std::string>& labels, std::vector<ObjectTrace>& objects, float thres_detect)
{
    int net = this->detector->executeSnpeMobilenetSSD(rawData, w, h, labels, objects, thres_detect);
    if(net != STATUS_SUCCESS)
    {
        LOG_FAIL("Execute Mobilenet SSD for object detection failed");
        return STATUS_FAILED;
