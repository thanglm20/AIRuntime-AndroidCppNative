...
int ObjectDetector::executeObjectDetector(const uint8_t* rawData, int w, int h, std::vector<ObjectTrace>& objects, float thres_detect)
{
    objects.clear();

    if ((rawData != NULL && rawData[0] == '\0') || thres_detect < 0.0 || thres_detect >= 1.0)
    {
        LOG_FAIL("Execute AI fail, please check your input again !!!");
        return STATUS_FAILED;
    }
    ...
    if(this->framework == "snpe")
    {
        if(this->snpe_detector->executeSnpeObjectDetector(rawData, w, h, this->labels, objects, thres_detect) != STATUS_SUCCESS)
        {
            LOG_FAIL("Execute snpe failed, Please check your modols");
            return STATUS_FAILED;
        }
